...
function fonctionPourIconeVerte(element) {
    ...
    // modifier status bdd livraison
    modifyStatus(element)
}
...
function modifyStatus(element) {
    let xhr = new XMLHttpRequest()

    let body = {
        restockID: element.getAttribute("id"),
        state: 2
    }
    ...
    xhr.onreadystatechange = function () {
        if(xhr.readyState == 4) {
            if(xhr.status == 200) {
                console.log("Supply status changed")
            } else {
                console.log("Can't modify supply status")
            }
        }
    }

    xhr.open("POST", "https://api.fuelsync.hertinox.fr/restock/setrestockstate.php", true) // TODO
